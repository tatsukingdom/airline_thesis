...
save "dta_clean/${dataset}_ASVX", replace

keep if (inlist(carrier1, "AS", "VX") & inlist(carrier2, "AS", "VX") & inlist(carrier3, "AS", "VX") & inlist(carrier4, "AS", "VX"))
save "dta_clean/${dataset}_ASVXonly", replace


***********************************************
*** 5.Preparation for Aggregation ~Airport~ ***
***********************************************
...
foreach m of local apts{
	...
	replace `m'_city = "SFF" if inlist(`m', "SFO", "OAK", "SJC")
	replace `m'_city = "BST" if inlist(`m', "BOS", "MHT", "PVD")
	...
	replace `m'_city = "DTR" if inlist(`m', "DTW", "FNT")
	replace `m'_city = "HST" if inlist(`m', "IAH", "HOU")
	replace `m'_city = "LSA" if inlist(`m', "LAX", "BUR", "LGB", "SNA", "ONT")
	replace `m'_city = "MAM" if inlist(`m', "MIA", "FLL", "PBI") 
	replace `m'_city = "NYT" if inlist(`m', "LGA", "EWR", "JFK", "HPN", "ISP")
	replace `m'_city = "TMP" if inlist(`m', "TPA", "PIE", "SRQ")
}
...
foreach m of local airports{
	gen apt_num_`m' = 1
	replace apt_num_`m' = 2 if inlist(`m', "OLD", "CNC", "CLV", "DFO", "DTR", "HST")
	replace apt_num_`m' = 3 if inlist(`m', "WDC", "SFF", "BST", "MAM", "TMP")
	replace apt_num_`m' = 5 if inlist(`m', "NYT", "LSA")
}
...
* Create ASVX specific datasets
keep if (inlist(carrier1, "AS", "VX") | inlist(carrier2, "AS", "VX") | inlist(carrier3, "AS", "VX") | inlist(carrier4, "AS", "VX"))
save "dta_clean/${dataset}_ASVX_city", replace

keep if (inlist(carrier1, "AS", "VX") & inlist(carrier2, "AS", "VX") & inlist(carrier3, "AS", "VX") & inlist(carrier4, "AS", "VX"))
save "dta_clean/${dataset}_ASVXonly_city", replace
